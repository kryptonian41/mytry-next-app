...
const PaymentMethodSelector = ({ onChange, defaultPaymentMode }: Props) => {
  const [paymentMode, setPaymentMode] = useState(defaultPaymentMode || CheckoutType.COD)
        ...
        Object.entries(CheckoutType).map(([label, value]) => {
          return (
            <label className={styles.radio} key={value}>
              <input name="radio" type="radio" value={value} checked={value === paymentMode} onChange={handleChange} />
              <span>{label}</span>
            </label>
          )
        })
