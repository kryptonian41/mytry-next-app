...
const OrderTile = ({ order, active = false, ...restProps }: OrdeTileProps) => {
  ...
  const handleRepeatClick = useCallback(async () => {
    ...
    items.forEach(product => itemQuantityMap.set(product.product_id, product.quantity))
  ...
  return <div className="grid grid-cols-12 border border-gray-500" {...restProps}>
    <div className="col-span-4 text-white grid place-items-center cursor-pointer" style={{
    ...
    {
      active && <div className="col-span-12 p-4">
        {order.line_items.map(item => {
          return <div className="flex">
            <span className="block flex-1 mt-2">{item.name} x {item.quantity}</span>
            <span className="block mt-2">{order.currency_symbol} {item.total}</span>
          </div>
        })}
        ...
        <div className="flex justify-end mt-6">
          <button className="text-white py-2 px-4 rounded-3xl text-sm" style={{
...
const OrderList = ({ orders }: OrderListProps) => {
  ...
  return (
    <div className="space-y-4">
      {orders.map(order => <OrderTile order={order} active={activeOrderId === order.id} onClick={() => setActiveOrderId(order.id)} />)}
    </div>
  )
}
